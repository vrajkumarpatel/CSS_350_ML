...
class Board:
    class Cell:
        ...
        tree = 3
        apple = 4
    def __init__(self,width,height,size=50, start_energy=10, start_fatigue=0):
        self.width = width
        self.height = height
        self.size = size+2
        ...
        self.pics = {
            "wolf" : imload('wolf.png',size-4),
            "apple" : imload('apple.png',size-4),
            "human" : imload('human.png',size-4)
        }
        self.frame_no = 0
        self.start_energy = start_energy
        self.start_fatigue = start_fatigue
        self.energy = start_energy
        self.fatigue = start_fatigue
        self.max_energy = 20
        self.max_fatigue = 10
        self.human = (0,0)

    def state(self):
        return (*self.human,self.energy,self.fatigue)
    ...
    def move(self,dpos,check_correctness=True):
        ...
        if self.is_valid(new_pos) or not check_correctness:
            self.human = new_pos
            self.energy = clip(0,self.max_energy,self.energy-1)
            self.fatigue = clip(0,self.max_fatigue,self.fatigue+1)
            if self.at() == self.Cell.apple:
                self.energy = clip(0,self.max_energy,self.energy+10)
                self.matrix[self.human] = self.Cell.empty
            if self.at() == self.Cell.tree:
                self.rest()

    def rest(self):
        self.fatigue = clip(0,self.max_fatigue,self.fatigue-2)
    ...
    def image(self,Q=None, energy_level=None, fatigue_level=None):
        ...
        for x in range(self.width):
            for y in range(self.height):
                ...
                if self.matrix[x,y] == Board.Cell.empty and Q is not None:
                    q_slice = Q[x,y]
                    if energy_level is not None and fatigue_level is not None:
                        q_slice = Q[x,y,energy_level,fatigue_level]
    ...
    def plot(self,Q=None, energy_level=None, fatigue_level=None):
        plt.figure(figsize=(11,6))
        plt.imshow(self.image(Q, energy_level, fatigue_level),interpolation='hanning')
    ...
    def walk(self,policy,save_to=None,start=None):
        ...
        while True:
            if save_to:
                ...
                self.frame_no+=1
            if self.at() == Board.Cell.wolf:
                return 1 if self.energy >= 5 and self.fatigue <= 5 else -1
